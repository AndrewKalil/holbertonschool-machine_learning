...
def determinant(matrix):
    ...
    errorList = 'matrix must be a list of lists'
    ...
    errorSquare = 'matrix must be a square matrix'
    for element in matrix:
        if len(element) != my_len:
            raise ValueError(errorSquare)
...
def minor(matrix):
    ...
    errorSquare = 'matrix must be a non-empty square matrix'
    my_len = len(matrix)
    if my_len == 1 and len(matrix[0]) == 0:
        raise ValueError(errorSquare)
